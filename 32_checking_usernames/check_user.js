...
var current_User = ['Rauf', 'Mehtab', 'Khurmi', 'Sabhee', 'Muddasir'];
var new_User = ['Asiya', 'Rauf', 'Jameel', 'Sabhee', 'Molvi_Abdul_shukoor', 'Khurmi', "Dolli", "Muddasir", "Habibi", "Mehtab", "Ronaldo"];
//for each methods use for exceses of arry values:
new_User.forEach(function (newUsername) {
    var lowerCase = newUsername.toLowerCase();
    if (current_User.map(function (c_User) { return c_User.toLowerCase(); }).includes(lowerCase)) {
        console.log("The Username ".concat(newUsername, " is not Available.please write diffrent username."));
    }
    else {
